...
EncryptedRpcChannel.prototype._onRpcResult = function (rpcResult, duration) {
    ...
    if (callback) {
        ...
        setTimeout(function () {
            callback(null, self.checkIfGzipped(rpcResult.result), duration);
        }, 100);
    }
};

EncryptedRpcChannel.prototype.checkIfGzipped = function (obj) {
    if (typeof obj !== 'boolean' && obj.instanceOf('mtproto.type.Gzip_packed')) {
        ...
        var unpacked = new Type({buffer: buffer}).deserialize();
        this._parser.parse(unpacked);
        return unpacked;
    }
    else return obj;
}
        ...
        this.constructor.super_.prototype._call.call(this, reqMsg, function (ex, result, duration) {
            try {
                if (ex) {
                    ...
                    if (directCallback) {
                        directCallback(null, self.checkIfGzipped(result), duration);
                    }
                }
            } catch (err) {
