...
if(process.env.HIDE_TELEGRAM_LOGS === 'true' ){
    logger = {
...
EncryptedRpcChannel.prototype._onRpcResult = function (rpcResult, duration) {
    ...
    if (callback) {
        ...
        setTimeout(function () {
            checkIfGzipped(rpcResult.result, function(unzippedResult){
                callback(null, unzippedResult, duration);
            });
        }, 100);
    }
};
function checkIfGzipped(obj, callback) {
    if (typeof obj !== 'boolean' && obj.instanceOf('mtproto.type.Gzip_packed')) {
        ...
        zlib.gunzip(packedData, function(err, buffer){
            logger.info("arguments after unzipping", arguments);
            logger.info("Buffer after gunzip [%s]", buffer.toString('hex'));
            var Type = tl.TypeBuilder.requireTypeFromBuffer(buffer);
            callback(new Type({buffer: buffer}).deserialize());
        });
    }
    else callback(obj);
}
        ...
        this.constructor.super_.prototype._call.call(this, reqMsg, function (ex, result, duration) {
            try {
                if (ex) {
                    ...
                    if (directCallback) {
                        checkIfGzipped(result, function(unzippedResult){
                            directCallback(null, unzippedResult, duration);
                        });
                    }
                }
            } catch (err) {
